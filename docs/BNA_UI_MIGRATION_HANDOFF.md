...
4. `Badge` 高度观感偏差：RN Android `Text` 默认度量通常会带原生字体的上下 padding，而 Flutter `Text` 更依赖显式 `TextStyle.height`。即使两边 `fontSize`、`paddingVertical` 一样，只要组件里有文本，最终高度和垂直居中观感也可能不一致。

结论：后续迁移时，必须把“视觉对齐”和“功能对齐”分开检查，不能混在一起凭感觉判断。同时不要默认“同 token = 同高度”，遇到含文本组件时，要单独检查字体行高、平台默认文本 padding、字重变化对高度观感的影响。
...
5. 图标大小、位置、间距是否一致
6. 文字字号、字重、行高是否一致
7. RN 原生文本默认 padding 与 Flutter 显式 `TextStyle.height` 是否造成额外高度差
8. 颜色、边框、背景、透明度是否一致
9. disabled/loading/error 的颜色语义是否一致
